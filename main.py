...
def register():
    ...
    if form.validate_on_submit():
        ...
        if User.query.filter_by(email=form.email.data).first():
            flash("That email already signed up, please log-in with that email.")
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not checked_user:
            ...
            return redirect(url_for('login'))
        elif not check_password_hash(pwhash=checked_user.password, password=password):
            flash("You typed wrong password, try again.")
            return redirect(url_for('login'))
        else:
